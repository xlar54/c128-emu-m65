...
C128Hook_OnDIRECTORY:
        jsr emu_print_init
        ; DIRECTORY command - read and display directory listing
        ; Uses host OPEN/CHKIN/CHRIN to read, prints to screen
        ; via emulated CHROUT (writing to guest keyboard buffer)
...
_dir_next_line:
        ; Read 2-byte next-line pointer
        jsr CHRIN
        sta _dir_next_lo
        jsr CHRIN
        sta _dir_next_hi

        ; Check for end of directory (next-line = $0000)
        lda _dir_next_lo
        ora _dir_next_hi
        beq _dir_done

        ; Read 2-byte block count (little-endian)
        jsr CHRIN
        sta _dir_blocks_lo
        jsr CHRIN
        sta _dir_blocks_hi
        ...
        ; Print space after block count
        lda #$20
        jsr emu_chrout

        ; Read and print filename/type until $00
_dir_print_chars:
        jsr CHRIN
        beq _dir_line_done      ; $00 = end of line
        jsr emu_chrout
        ; Check READST for errors
        pha
        jsr READST
        and #$40                ; EOF?
        bne _dir_eof
        pla
        bra _dir_print_chars

_dir_eof:
        pla
_dir_line_done:
        ; Render the screen so chars become visible
        ; Check READST for EOF
        jsr READST
        and #$40
        bne _dir_done

        ; Check for RUN/STOP key (MEGA65 $D610)
        lda $D610
        beq _dir_next_line      ; no key pressed
        sta $D610               ; acknowledge
        cmp #$03                ; RUN/STOP = $03
        beq _dir_done
        bra _dir_next_line
...
_dir_print_blocks:
        ; Convert 16-bit _dir_blocks to decimal and print
        ; Simple approach: divide by 10000, 1000, 100, 10, 1
        lda _dir_blocks_lo
        sta _dir_tmp_lo
        lda _dir_blocks_hi
        sta _dir_tmp_hi
        lda #0
        sta _dir_leading_zero   ; suppress leading zeros
...
_dir_print_digit:
        cpx #0
        bne _dir_nonzero
        lda _dir_leading_zero
        beq _dir_skip_digit     ; skip leading zero
_dir_nonzero:
        lda #1
        sta _dir_leading_zero
        txa
        ora #$30
        jsr emu_chrout
_dir_skip_digit:
